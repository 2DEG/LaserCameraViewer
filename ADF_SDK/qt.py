...
class MainWindow(QMainWindow):
	...
	def __init__(self):
		...
		self.lbl_temp = QLabel(str(adfcam.ADFCAM_TEMP_DEF))
		self.lbl_tint = QLabel(str(adfcam.ADFCAM_TINT_DEF))
		self.slider_temp = QSlider(Qt.Horizontal)
		self.slider_tint = QSlider(Qt.Horizontal)
		self.slider_temp.setRange(adfcam.ADFCAM_TEMP_MIN, adfcam.ADFCAM_TEMP_MAX)
		self.slider_temp.setValue(adfcam.ADFCAM_TEMP_DEF)
		self.slider_tint.setRange(adfcam.ADFCAM_TINT_MIN, adfcam.ADFCAM_TINT_MAX)
		self.slider_tint.setValue(adfcam.ADFCAM_TINT_DEF)
	...
	def startCamera(self):
		self.pData = bytes(adfcam.TDIBWIDTHBYTES(self.imgWidth * 24) * self.imgHeight)
		...
		if self.cur.model.flag & adfcam.ADFCAM_FLAG_MONO == 0:
		...
		try:
			...
			self.slider_temp.setEnabled(self.cur.model.flag & adfcam.ADFCAM_FLAG_MONO == 0)
			self.slider_tint.setEnabled(self.cur.model.flag & adfcam.ADFCAM_FLAG_MONO == 0)
	...
	def openCamera(self):
		self.hcam = adfcam.Adfcam.Open(self.cur.id)
		if self.hcam:
	...
	def handleStillImageEvent(self):
		...
		try:
			...
			if info.width > 0 and info.height > 0:
				buf = bytes(adfcam.TDIBWIDTHBYTES(info.width * 24) * info.height)
				try:
