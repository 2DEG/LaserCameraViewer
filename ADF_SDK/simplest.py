...
# the vast majority of callbacks come from adfcam.dll/so/dylib internal threads
	@staticmethod
	def cameraCallback(nEvent, ctx):
		if nEvent == adfcam.ADFCAM_EVENT_IMAGE:
	...
	def run(self):
		...
		if len(a) > 0:
			...
			self.hcam = adfcam.Adfcam.Open(a[0].id)
			if self.hcam:
				try:
					width, height = self.hcam.get_Size()
					bufsize = adfcam.TDIBWIDTHBYTES(width * 24) * height
