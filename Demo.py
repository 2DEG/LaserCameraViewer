...
def getExposureTime(camera):
    ...
    nRet = exposureTimeNode.contents.getValue(exposureTimeNode, gain)
    print(gain.contents)
    gain = int.from_bytes(gain, "big")
    
    print(nRet)
    if ( nRet != 0 ):
        print("get ExposureTime value [%f]us fail!"  % (gain))
        # 释放相关资源
        exposureTimeNode.contents.release(exposureTimeNode)
        return -1
    else:
        print("get ExposureTime value [%f]us success." % (gain))
...
def demo():    
    ...
    nRet = getExposureTime(camera)
    if ( nRet != 0 ):
        print("get ExposureTime fail")
        # 释放相关资源
        streamSource.contents.release(streamSource)   
        return -1
